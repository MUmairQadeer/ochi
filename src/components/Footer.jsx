...
function Footer() {
  return (
    <div> 
    <div className='w-full h-[70vh] bg-zinc-900 p-20 flex'>
        <div className='w-1/2 flex flex-col h-full justify-between'>
           <div className="heading">  
              <h1 className='text-[6vw] tracking-tighter semi-bold leading-none -mb-5 uppercase '>Eye-</h1>
              <h1 className='text-[6vw] tracking-tighter  semi-bold leading-none -mb-5 uppercase '>Opening</h1>
           </div>
        ...
        <h1 className='text-[6vw] tracking-tighter semi-bold leading-none -mb-5 uppercase '>Presentations</h1>
        <div className='flex flex-col gap-2 py-6'>S: 
        {["Instagram" ,"Behance" ,"Facebook" ,"Linkedin"].map((item,index)=>{
          return <p key={index} className='underline underline-offset-4'>{item}</p>
        })}
        </div>
        <div className="mid flex items-center justify-between"> 
        <div className='flex flex-col gap-2 py-6'>M: 
        {["Home" ,"Services" ,"Our Work" ,"About Us" ,"Insights" ,"Contact us"].map((item,index)=>{
          return <p key={index} className='underline underline-offset-4'>{item}</p>
        })}
        </div>
        <div className='flex flex-col gap-2 py-6'>L : 
        {["202-1965 W 4th Ave" ,"Vancouver, Canada" ,"30 Chukarina St" ,"Lviv, Ukraine" ,"Insights" ,"Contact us"].map((item,index)=>{
          return <p key={index} className='underline underline-offset-4'>{item}</p>
        })}
        </div>
        </div>
        ...
        <div className="final flex  h-full w-full items-center justify-between">
          {["OCHI" ,"© ochi design 2024. Legal Terms" ,"Website by Obys"].map((item,index)=>{
            return <p key={index} className='mb-5 px-[4vw] '>{item}</p>
          })
        }
        </div>
    </div>
